...
Tools: Executable functions that agents can call. (e.g., generate\_image(), send\_transaction()). The Agent "Action" system is built on invoking these Tools.
Prompts: Reusable templates that structure interactions. (e.g., analyze\_sentiment, extract\_topics). These standardize the internal reasoning processes.
...
Calls the mcp-server-weaviate tool named search\_memory to find the 5 most semantically relevant past memories based on the input\_query.
...
Include a list\_resources handler that advertises this resource to the client.
...
Act: Worker calls twitter.reply\_tweet Tool.
...
In the \_\_init\_\_ method, initialize CdpEvmWalletProvider using environment variables CDP\_API\_KEY\_NAME and CDP\_API\_KEY\_PRIVATE\_KEY.
Implement an async method send\_payment(to\_address: str, amount\_usdc: float) that uses the erc20\_action\_provider to transfer USDC on the Base network.
Add a custom decorator @budget\_check to this method. This decorator should check a local Redis key daily\_spend. If daily\_spend + amount\_usdc > MAX\_DAILY\_LIMIT, raise a BudgetExceededError and do not execute the transaction.
If the transaction succeeds, atomically update the daily\_spend in Redis."
...
Include two action buttons: 'Approve' (POSTs to /api/approve/{task\_id}) and 'Reject' (POSTs to /api/reject/{task\_id}).
...
Planner Service: Write a Python service that reads a goals.json file, decomposes it using a mock LLM call, and pushes Task objects to a Redis queue named task\_queue.
Worker Service: Write a Python service that pops tasks from task\_queue, simulates work (async sleep 2s), and pushes a Result object to a Redis queue named review\_queue.
Judge Service: Write a Python service that pops from review\_queue, checks if result.status == 'success', and logs the outcome.
...
Implement a method call\_tool(server\_name, tool\_name, arguments) that sends a standard JSON-RPC request to the connected server.
...
Create an Action Provider class that exposes transfer\_asset and get\_wallet\_balance functionality.
...
I asked a developer for vibe coding best practices and this what they shared. - Reddit, accessed February 4, 2026, https://www.reddit.com/r/vibecoding/comments/1l8r502/i\_asked\_a\_developer\_for\_vibe\_coding\_best/
...
Cursor conducted an experiment in which hundreds of agents collaborated to build a browser from scratch, writing 1,000 files and over 1 million lines of code in a week. - GIGAZINE, accessed February 4, 2026, https://gigazine.net/gsc\_news/en/20260120-cursor-scaling-agents/
